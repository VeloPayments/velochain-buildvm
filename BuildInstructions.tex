...
tty7::respawn:/sbin/getty -L ttyS0 115200 vt100
...
\subsection{Building GRUB}

We will use GRUB to install the bootloader for bootstrapping Linux. Although
this is a system dependency, the GRUB tool actually installs the requisite
binaries to the boot partition when we run it. So, there is no need to install
this in the OS image itself.  Instead, we will install it into the cross-tools
directory.

<<cross-build-grub>>=
#build grub
if [ ! -f ${GRUB_PKGFILE}.built ]; then
    cd ${GRUB_DIR}
    mkdir build
    cd build
    ../configure --prefix=${OSDIR}/cross-tools
...
\section{Building System Binaries}

Now we can build the Linux kernel and all needed system binaries. We will use
the cross compilation tools we built during the previous section, as well as GNU
libc.  Once these are built, we can create the QEmu image, mount it, partition
it, format it, and install these system binaries.  Additionally, we can install
any optional packages we wish to include in this minimal OS image.
...
\subsection{Building busybox}

We can now build busybox with the new cross compiler tools.  We will use busybox
to provide most of the userland we will need in this VM.

<<build-busybox>>=
#build busybox
if [ ! -f ${BUSY_PKGFILE}.built ]; then
    cd ${BUSY_DIR}
    make CROSS_COMPILE="${OS_TARGET}-" defconfig
    make -j${CORES} CROSS_COMPILE="${OS_TARGET}-"
    make CROSS_COMPILE="${OS_TARGET}-" CONFIG_PREFIX="${OSDIR}" install
    cp -v examples/depmod.pl ${OSDIR}/cross-tools/bin
    chmod 755 ${OSDIR}/cross-tools/bin/depmod.pl
    cd ${WD}
    touch ${BUSY_PKGFILE}.built
else
...
We will build a "good enough" configuration of the Linux kernel.  I have added
some tweaks to remove a few drivers that cause OpenBSD's VMD some issues.  Since
these drivers are not needed by QEMU, this won't be an issue.

<<build-linux-kernel>>=
#build the Linux kernel
if [ ! -f ${LINUX_PKGFILE}.built ]; then
    cd ${LINUX_DIR}
    #create the default config
    make ARCH=${OS_ARCH} CROSS_COMPILE=${OS_TARGET}- x86_64_defconfig
    #back up this default config
    cp .config .config-backup
    #remove the INTEL_PSTATE driver, as this doesn't work in OpenBSD VMD
    #also, remove a bunch of other unnecessary things.
    cat .config-backup | sed '/CONFIG_X86_INTEL_PSTATE/d' > .config
    make -j${CORES} ARCH=${OS_ARCH} CROSS_COMPILE=${OS_TARGET}-
    make ARCH=${OS_ARCH} CROSS_COMPILE=${OS_TARGET}- \
        INSTALL_MOD_PATH=${OSDIR} modules_install
    cp -v arch/x86/boot/bzImage ${OSDIR}/boot/vmlinuz-${LINUX_VERSION}
    cp -v System.map ${OSDIR}/boot/System.map-${LINUX_VERSION}
    cp -v .config ${OSDIR}/boot/config-${LINUX_VERSION}
    mkdir -pv ${OSDIR}/lib/modules/${LINUX_VERSION}
    ${OSDIR}/cross-tools/bin/depmod.pl \
        -F ${OSDIR}/boot/System.map-${LINUX_VERSION} \
        -b ${OSDIR}/lib/modules/${LINUX_VERSION}
    touch ${LINUX_PKGFILE}.built
else
    echo "linux kernel already built."
fi
>>@<<

\section{Creating the ISO Image}

One of the shortcuts we will be making in order to build the final VM image is
to set up a bootable ISO image, which we can boot from QEMU, in order to create
the final image.  It is possible to use the block device to mount QEMU images
directly to partition and format them.  However, scripting this is problematic
since there is no way to predict what sort of block devices are being used by a
particular Linux system.  Thinking this over, I had nightmares of accidentally
formatting someone's primary partition.  If we are going to do low-level
formatting of block devices, I'd rather do this from a virtual machine to give
some level of guarantee that I'm not going to harm someone's development system.
Therefore, we will take advantage of a feature in GRUB, which is the ability to
create a bootable rescue ISO.  We will use this rescue ISO to format and build
the final image from a temporary QEMU virtual machine.

We will create a copy of the OS image to use for installation so we can perform
some manipulation of it.  Then, we will set the correct permissions, and use
/verb/grub-mkrescue/ to create an ISO. Thus, we can boot from QEMU in order to
produce the final image.

<<FILE:build.sh>>=
    <<image-copy-os-directory>>
    <<image-fixup-os-directory>>
    <<image-create-iso>>
>>@<<

\subsection{Copying the OS directory}

As not to clobber the OS directory, we will remove the old copy of the OS
directy -- if it exists -- and create a new copy OS directory.

<<image-copy-os-directory>>=
    sudo rm -rf ${OSDIR}-copy
    cp -rf ${OSDIR}/ ${OSDIR}-copy
>>@<<

\subsection{Applying Fixups to the OS directory}

First, we will delete any files we do not need, then we will strip all files and
update file permissions.

<<image-fixup-os-directory>>=
    rm -rfv ${OSDIR}-copy/cross-tools
    rm -rfv ${OSDIR}-copy/usr/src/*
    for file in $(ls ${OSDIR}-copy/usr/lib64/*.a); do
        echo $file
        rm -f $file
    done
    find ${OSDIR}-copy/{,usr/}{bin,lib,sbin} \
        -type f -exec strip --strip-debug '{}' ';'
    find ${OSDIR}-copy/{,usr/}lib64 \
        -type f -exec strip --strip-debug '{}' ';'
    sudo chown -R root:root ${OSDIR}-copy
    sudo chgrp 13 ${OSDIR}-copy/var/run/utmp ${OSDIR}-copy/var/log/lastlog
    sudo mknod -m 0666 ${OSDIR}-copy/dev/null c 1 3
    sudo mknod -m 0600 ${OSDIR}-copy/dev/console c 5 1
    sudo chmod 4755 ${OSDIR}-copy/bin/busybox
>>@<<

\subsection{Creating the ISO image}

Finally, we will create the iso image using GRUB.

<<image-create-iso>>=
sudo grub-mkrescue -o linux.iso linux-copy
>>@<<
